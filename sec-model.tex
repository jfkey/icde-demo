...
Central to \oursystem is our ranking model that supports heterogeneous scholarly entity ranking using various ranking metrics.
We next introduce the details of its ranking model.

\subsection{Ranking Metrics}
\label{subsec:rankingMetric}
To fit the various ranking scenarios, \oursystem is equipped with four ranking metrics, namely {\em relevance}, {\em importance}, {\em citation} and {\em time}. We first elaborate how to rank scholarly articles with these metrics and extend them for other types of entities in the next subsection.
%
...
\stitle{Sorted by relevance}. The first metric enables to query highly-correlated scholarly articles. For doing so, we evaluate the relevance of their titles given the query keywords, and return those with the highest relevance. Distributed word representations~\cite{corrado2013efficient} are employed to capture semantic-level relevance defined as follows:
\begin{equation}
\label{eq:simscore}
rel(a.T, Q) = \sum_{t \in a.T} \sum_{q \in Q} idf(q) \frac {\textbf{t} \cdot \textbf{q}} {|| \textbf{t} ||\ ||\textbf{q}||}.
\end{equation}
...
\vspace{-1ex}
\caption{\small \sarank Framework~\cite{ma2018query}} \label{fig:sarank}
\vspace{-3ex}
\end{figure}

\stitle{Sorted by importance}. The second metric for querying important scholarly articless is adapted from a recently developed scholarly article ranking model \sarank~\cite{ma2018query}. The importance of scholarly entities is defined as a combination of prestige and popularity, where prestige is computed by a novel Time-Weighted PageRank and popularity is evaluated as the sum of citation freshness. To assign those newly published articles reasonable ranking scores, it further assembles the importance of citations, authors and venues to derive the final ranking scores, as illustrated in Fig.~\ref{fig:sarank}.

\stitle{Sorted by citation and time}. For the remaining two metrics, we simply sort scholarly articles based on their numbers of total citations and their publish time, respectively. And ties are broken by further comparing importance scores.
