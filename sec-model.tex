...
Central to \oursystem is our ranking model that supports heterogeneous scholarly entity ranking with four metrics.
We next introduce the details of our ranking model.

\subsection{Ranking Metrics}
\label{subsec:rankingMetric}
Our \oursystem is equipped with four ranking metrics, namely {\em relevance}, {\em importance}, {\em citation number} and {\em time}, to fit the various ranking scenarios. We first elaborate how to rank scholarly articles with these metrics and leave the extension for other entities in the next subsection.

\stitle{Sorted by relevance}. With the first metric, we first evaluate 

 it is vital to evaluate the similarity between the short query and sentence (\ie title of the paper) when retrieve articles by keywords.
...
\stitle{Sorted by importance}. 

\stitle{Sorted by citation number and time}. For the remaining two metric, we simply sort articles based on their numbers of total citations and their publish time, respectively. And ties are broken by further comparing importance scores.
