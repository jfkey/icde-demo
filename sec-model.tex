...
\end{equation}
Here $a.T$ and $Q$ are the sets of words (excluding stop words) in the title of an article $a$ and the query, respectively, $idf(q)$ is the inverse document frequency of a word $q$, and $\textbf{t}$ and $\textbf{q}$ are their corresponding word embeddings. \marked{Although the entire article to compute the relevance can improves relevance accuracy, it is not the main concern of our work, we just briefly touch on this and will explore it in future work.}
