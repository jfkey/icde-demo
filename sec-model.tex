...
Central to \oursystem is its ranking model that supports heterogeneous scholarly entity ranking with five ranking metrics.
In this section, we introduce the details of rankings.

\subsection{Ranking Metrics}
\label{subsec:rankingMetric}
To fit various ranking scenarios, \oursystem implements five ranking metrics, namely {\em relevance}, {\em importance}, {\em relevant importance}, {\em citation} and {\em time}. We first elaborate how to rank scholarly articles with these metrics and extend to other entities in Section~\ref{subsec:heteroRanking}.
%
Note that given a query, those articles whose titles contain at least one query keyword after removing stop words are retrieved as candidates for ranking.

\stitle{Sort by relevance}. The first metric enables to query highly-correlated scholarly articles. Thus, we evaluate the relevance of their titles given the query, and return those with the highest relevance. Since the development of Word2vec~\cite{corrado2013efficient}, word embedding has become the {\em de facto} standard method to capture semantic-level relevance. We also exploit Word2vec for evaluating the relevance defined as follows:
\begin{equation}
\label{eq:relscore}
...
\stitle{Sort by importance}. The second metric for querying important scholarly articles is adopted from a recently developed scholarly article ranking model \sarank~\cite{ma2018query}. The importance is defined as a combination of prestige and popularity, where prestige is computed by a novel Time-Weighted PageRank and popularity is evaluated as the sum of citation freshness. To assign those newly published articles reasonable ranking scores, it further assembles the importance of citations, authors and venues to derive the final ranking, as illustrated in Fig.~\ref{fig:sarank}.

\stitle{Sort by relevant importance.} The importance metric does not consider how the articles relate to queries. The third metric is then proposed for retrieving scholarly articles both relevant and important. More specifically, we first normalize the computed relevance (resp. importance) scores by scaling with the maximal relevance (resp. importance) scores in the candidate set, and then rank scholarly articles based on the following relevant importance scores:
\begin{equation}
\label{eq:relimp}
rImp(a, Q) = \alpha \cdot rel_n(a, Q) + (1-\alpha)\cdot imp_n(a),
\end{equation}
where $rel_n(a, Q)$ and $imp_n(a)$ are the {\em normalized} relevance and importance scores, and $\alpha$ is a regularizing parameter.  We fix $\alpha=0.5$ by default.

\stitle{Sort by citation and time}. For the remaining two metrics, scholarly articles are simply sorted based on their numbers of total citations and their publish time, respectively. And ties are broken by further comparing importance scores.


\subsection{Heterogeneous Scholarly Entity Ranking}
\label{subsec:heteroRanking}

Besides scholarly articles, other scholarly heterogeneous entities are another important part of the scholarly data.
Our \oursystem further supports ranking these heterogeneous entities, \ie authors, venues, and affiliations.
...
These rankings are derived based on the ranking scores of scholarly articles. And the ranking score of an entity is computed as the sum of the ranking scores of its associated articles. When sorting by time, we use an exponentially decayed ranking score $e^{T_a-T_0}$ where $T_a$ and $T_0$ are the publish year of article $a$ and the current year, respectively.
