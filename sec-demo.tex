...
\begin{figure*}[tp]
\centering
\includegraphics[width=\textwidth]{searchKeywords2.pdf}
\vspace{-3.5ex}
...
When setting radio option checked and sorted by Rel.Imp (relevant importance), the rankings of articles sorted by  \sarank and PageRank score and the associated entities (authors, affiliations, conferences and journals) are shown in the middle and on the right-hand side.
...
\marked{
As shown in the middle of Figure~\ref{fig:searchKeywords}, we evaluate the ranking quality by compared with PageRank. Specifically, we put the article ``MillWhell: Fault-tolerant Stream Processing At Internet Scale"(millwhell) in the first place sorted by \sarank while ``Adaptive Control of Extreme-Scale Stream Processing Systems"(adaptive) in the first place sorted by PageRank.
Although ``adaptive" has a little more citations, ``millwhell" gets published in ``VLDB" which has a higher impact than ``ICDCS". Moreover,  ``millwhell" published in ``2013" more recently than ``adaptive" in ``2006" as researchers potentially discover recent influential articles.
Thus, our query result is more effective for searching influential articles due to considering the temporal factor and and assembling the venue and author component. 
}
