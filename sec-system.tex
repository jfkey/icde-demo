...
It contains seven basic types of nodes including paper, author, affiliation, venue, field of study (FOS), conference instance (ConIns) and year. In addition, it further incorporates an artificial type of nodes, \ie PAA representing paper-author-affiliation tuples. This enhances query efficiency, \ie an author and her/his affiliation can be retrieved by processing one query.
...
Based on the above schema, we store the MAG data~\cite{sinha2015overview} as a huge property graph with more than one billion nodes and over two billion relationships \marked{(renjun Note: replacing one/two billion with concrete numbers).}
