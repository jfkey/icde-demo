...
Based on the above schema, we store our scholarly data, \ie MAG~\cite{sinha2015overview}, as a huge property graph with more than one billion nodes and over two billion relationships \marked{(Note: replacing one/two billion with concrete x.y billion).} We further clarify our graph storage with the following two points.
...
\oursystem supports a variety of common queries on the property graph. And the graph {\em Query Engine} is responsible for processing this set of queries. When users issue a query, \oursystem first translates it into a cypher \marked{check if it is this correct?}. The cypher is then sent to the {\em Neo4j Query Engine}, which generates a query plan after proper parsing and semantic analysis.
...
An example of finding the top scholarly articles on ``graph database"  ranked by relevant importance is shown in Table II.
\marked{Note This is an example of what? cypher or others}
