...
Based on the above schema, we store our scholarly data, \ie MAG~\cite{sinha2015overview}, as a huge property graph with more than one billion nodes and over two billion relationships \marked{(renjun Note: replacing one/two billion with concrete x.y billion).} We further clarify our graph storage with the following two points.
