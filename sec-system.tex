...
Moreover, both citation counts and importance scores support incremental computation~\cite{ma2018query}, and they are easy to be dynamically maintained \marked{once scholarly data get increment.}%once the property graph gets updated.
...
\marked{
Utilizing {\em Neo4j query engine}, \oursystem supports a variety of graph queries on the property graph to aid scholarly search, such as searching the author's top-k fields of study and searching the affiliation having higher impact on which venues. When a query is issued, \oursystem first translates it into a Neo4j Cypher query. 
